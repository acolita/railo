...
/**
 * Die Klasse CFMLTransformer ist das Herzstï¿¼ck des ï¿œbersetzungsprozess, 
 * es liest die ï¿¼bergebene CFML Datei ein und ï¿¼bersetzt diese in ein valid (CFXD) XML Dokument 
 * in der Form eines org.w3c.dom.Document Object, 
 * die dann als weitere Vorlage zum ï¿¼bersetzten in PHP dient.
 * Der CFMLTransformer ï¿¼bersetzt nur die Tags die innerhalb einer CFML Seite vorkommen, 
 * nicht die Ausdrï¿¼cke die innerhalb von Attributen und dem Body eines Tag vorkommen kï¿¶nnen, 
 * fï¿¼r dies ist der ExprTransformer zustï¿¤ndig, 
 ...
 * <pre>
		...
		comment		= "<!---" {?-"--->"} "--->";
		literal		= ("<" | {?-"#"-"<"} "<" | {"#" expression "#"} "<" ) | ({?-"<"} "<")
				  (* Welcher Teil der "oder" Bedingung ausgefï¿¼hrt wird, ist abhï¿¤ngig was die Tag-Lib vorgibt, 
				     dass Expression geparst werden sollen oder nicht. *)
		tag		= name-space identifier spaces attributes ("/>" | ">" [body "</" identifier spaces ">"]);
				  (* Ob dem Tag ein Body und ein End-Tag folgt ist abhï¿¤ngig von Definition des body-content in Tag-Lib, gleices gilt fï¿¼r appendix *)
		...
		attributes	= ({spaces attribute} "/>" | {spaces attribute} ">") | attribute-value;
				  (* Welcher Teil der "oder" Bedingung ausgefï¿¼hrt wird, ist abhï¿¤ngig von der Tag Attribute Definition in der Tag Lib. *)
...
public final class CFMLTransformer {
	 ...
	 * @param config
	 * @param sf CFML File
	 * @param tlibs Tag Library Deskriptoren, nach denen innerhalb der CFML Datei geprï¿¼ft werden soll.
	 * @param flibs Function Library Deskriptoren, nach denen innerhalb der Expressions der CFML Datei geprï¿¼ft werden soll.
	 * @return ï¿œbersetztes CFXD Dokument Element.
	 ...
	 * @param parseExpression Definiert ob Expressions innerhalb von Literalen ï¿¼bersetzt werden sollen oder nicht.
	 * @param transformer Expression Transfomer zum ï¿¼bersetzten von Expression.
	 * @throws TemplateException
	 */
	...
	/**
	 * Liest einen Kommentar ein, Kommentare werden nicht in die CFXD ï¿¼bertragen sondern verworfen.
	 * Komentare kï¿¶nnen auch Kommentare enthalten.
	 * <br />
	 * EBNF:<br />
	 * <code>"<!---" {?-"--->"} "--->";</code>
	 * @throws TemplateException
	 ...
	 * beim Einlesen wird unterschieden ob Expression geparsst werden mï¿¼ssen oder nicht, 
	 * dies ist abhï¿¤ngig, von der Definition des Tag in dem man sich allenfalls befindet, innerhalb der TLD.
	 * @param parent ï¿¼bergeordnetes Element.
	 * @param parseExpression Definiert on Expressions geparset werden sollen oder nicht.
	 * @param transformer Expression Transfomer zum ï¿¼bersetzen der Expressions innerhalb des Literals.
	 * @throws TemplateException
	 * 
	 * <br />
	 * EBNF:<br />
	 * <code>("<" | {?-"#"-"<"} "<" | {"#" expression "#"} "<" ) | ({?-"<"} "<")
			(* Welcher Teil der "oder" Bedingung ausgefï¿¼hrt wird, ist abhï¿¤ngig ob die Tag-Lib vorgibt, 
	...
	/**
	 * Liest einen Tag ein, prï¿¼ft hierbei ob das Tag innerhalb einer der geladenen Tag-Lib existiert, 
	 * ansonsten wird ein Tag einfach als literal-string aufgenommen.
	 * <br />
	 * EBNF:<br />
	 * <code>name-space identifier spaces attributes ("/>" | ">" [body "</" identifier spaces ">"]);(* Ob dem Tag ein Body und ein End-Tag folgt ist abhï¿¤ngig von Definition des body-content in Tag-Lib, gleices gilt fï¿¼r appendix *)</code>
	 * @param parent ï¿¼bergeornetes Tag
	 * @param parseExpression sollen Expresson innerhalb des Body geparste werden oder nicht.
	 * @return Gibt zurï¿¼ck ob es sich um ein Tag as einer Tag-Lib handelte oder nicht.
	 * @throws TemplateException
	 */
...
//		TODO muss erlaubt sein
	 ...
	 * gibt eine Tag-Lib zurï¿¼ck falls eine passt, ansonsten null. 
	 * <br />
	 * EBNF:<br />
	...
	/**
	 * Liest die Attribute eines Tags ein, dies Abhï¿¤ngig von der Definition innerhalb der Tag-Lib.
	 * Hierbei unterscheiden wir vier verschiedene Arten von Attributen:<br>
	 * <ul>
	 * <li>FIX: Definierte Attribute Fix, fï¿¼r jedes Attribut ist definiert ob es required ist oder nicht (gleich wie JSP). </li>
	 * <li>DYNAMIC: Die Attribute des Tag sind frei, keine Namen sind vorgegeben. 
	 * Es kann aber definiert sein wieviele Attribute maximal und minimal verwendetet werden dï¿¼rfen.</li>
	 ...
	 * <br />
	 * EBNF:<br />
	 * <code>({spaces attribute} "/>" | {spaces attribute} ">") | attribute-value;(* Welcher Teil der "oder" Bedingung ausgefï¿¼hrt wird, ist abhï¿¤ngig von der Tag Attribute Definition in der Tag Lib. *)</code>
	 * @param tag
	 * @param parent
	 * @throws TemplateException
	 */
     ...
     * @param tag Definition des Tag das dieses Attribut enthï¿¤lt.
     * @param args Container zum Speichern einzelner Attribute Namen zum nachtrï¿¤glichen Prufen gegen die Tag-Lib.
	 ...
	 * wird der Name ï¿¼ber den identifier oder den Expression Transformer eingelesen.
	 * <ul>
	 ...
	 * @param isNonName
	 * @return Element Eingelesener ï¿¼bersetzer Wert des Attributes.
	 * @throws TemplateException
	 */
	...
	/**
	 * Liest einen Identifier ein und gibt diesen als String zurï¿¼ck.
	  * <br />
	 * EBNF:<br />
